...
- Local API制御pollはマイク会話オフ時5秒、マイク会話ONの待機・モデル準備時500ms、録音・文字起こし・送信中など低遅延が必要なphaseだけ100msを上限としてください。100msをマイク会話ON全体の既定値にしません。
- マイク会話ON時のMV3 keepaliveは表示中ChatGPTタブだけ20秒間隔で許可し、非表示タブでは停止してください。全タブheartbeatへ拡張しません。
...
Local Voice Bridgeの通常runtimeまたはセットアップに、別リポジトリのアプリ／runtime、別常駐プロセス／アプリ、新しいlocalhostサービス／ポート、新しいインストール・起動・監視手順、外部runtime依存、または製品目的外の利用者機能を追加する変更は、実装前に利用者の明示承認が必要です。承認がない場合は実装しないでください。「既存ツールを再利用できる」「責務分離がきれいになる」「別ツールの不具合を直せる」は承認を省略する理由になりません。通常利用者が管理するものが増える変更は内部変更として扱いません。

`scripts/check-runtime-boundaries.js`はこの境界の機械的な補助検査です。検査を弱めて依存を追加するのではなく、必要なアーキテクチャ変更なら先に明示承認と仕様変更を行ってください。
