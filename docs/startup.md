...
`Voice`は既存の参照音声設定`referenceVoice`の表示名です。選択したIDと同名のペット素材がある場合はデスクトップペットも連動します。`詳細設定`は設定を小窓へ複製せず、拡張機能が所有する読み上げ範囲・文字起こしモデル・送信前猶予などの設定画面を開きます。
...
ペットの種類はWindows Local Voice小窓の`Voice`と自動連動します。Chrome内にはペット本体や`Pet`選択欄を表示しません。`Voice=none`または空の場合と、同じIDの素材がない場合は既定ペットへフォールバックします。
...
5. 小窓に`Voice`、`Volume`、`マイク会話`、`Auto`、`Next`、`Regen`、`Stop`、`Replay`、`詳細設定`があり、Chrome内には操作パネルがないことを確認する
6. `詳細設定`を押して拡張機能所有の設定画面が開くことを確認する
7. Chrome / Braveで2つ以上のChatGPTタブを開き、小窓で`Auto`をオンにする
8. 各タブで新しい返答を生成し、両方の返答が1つの共通キューで順番に読み上げられることを確認する
9. `思考中`だけの途中状態は読まず、最終返答だけを読むことを確認する
10. `Voice`を変更し、Chrome保存値、読み上げ参照音声、同じIDのデスクトップペットが一致することを確認する
11. ペットの左ドラッグで位置を移動でき、ドラッグ終了直後に小窓が誤って開かないことを確認する
12. 小窓の×で非表示にし、ペットのダブルクリックで再び開くことを確認する
13. `Exit`で終了し、再起動後に小窓位置、ペット位置、`Voice`、`Volume`、`Auto`が復元されることを確認する
