...
- Autoをオンにした後の新しい返答だけを監視し、返答完了を確認してから初期値では最大2行・80文字の冒頭を一度だけ再生（拡張機能のオプションで変更可能）
...
読み上げる最大行数・最大文字数、文字起こしモデル、文字起こし後の送信前猶予、マイクLiveのTTSプロファイル（`speed` / `balanced` / `bridge`）は、Chrome / Braveの拡張機能アイコンを右クリックして`オプション`を開き、設定します。`マイク会話`のオン・オフは、これまでどおりWindows Local Voice小窓で操作します。
...
最初は`Ref=none`のまま、Windows Local Voice小窓でAutoをオンにしてから新しいメッセージを送ります。開いているどのChatGPTタブでも、推論完了後に新しい返答の先頭プレビューが一度だけ再生されれば完了です。参照音声の追加方法は[参照音声](docs/reference-audio.md)を参照してください。

双方向の音声会話を使う場合は、セットアップ画面で`読み上げ + マイク会話`を完了してから、小窓の`マイク会話`をオンにします。送信先にしたい入力欄へフォーカスを入れ、右Ctrl＋右Shift左の`＼ / _`キーを押している間だけ録音します。どちらか一方を離すとCUDAで日本語をローカル文字起こしし、録音開始時にフォーカスしていた入力欄へ表示します。送信前の関連付けをローカルへ保存できた場合だけChatGPTへ送信し、返答候補が一意に確定すると、句読点で確定した文からSuguha音声を逐次再生します。初期値では0.7秒以内にEscを押すと今回挿入した文字だけを削除します。読み上げ中の入力、Enter、送信ボタン、Regen、ページ遷移、次の録音で古い音声を即座に失効します。CUDAに失敗した場合はCPUへ黙って切り替えず、エラーとして停止します。詳しい準備、状態表示、保存データ、制約は[操作と検証](docs/operation.md#マイク会話モード)を参照してください。
