...
日常操作はWindows小窓の`Voice`、`Volume`、`マイク会話`、`Auto`、`Next`、`Regen`、`Stop`、`Replay`で行います。`Voice`は既存の参照音声選択`referenceVoice`の表示名で、同じIDのペット素材があればデスクトップペットも連動します。読み上げ量や文字起こしモデルなどは小窓の`詳細設定`から拡張機能所有の設定画面を開いて変更します。マイク会話を使う場合はセットアップで追加機能を導入し、送信先の入力欄へフォーカスして指定キーを押している間だけ録音します。拡張機能の切断または更新待ちを検出した場合は、小窓に`拡張機能を再読み込み`ボタンを表示します。切断中は再読み込み対応可否を断定せず、loopbackの再読み込み要求を待ち行列へ入れて拡張機能の再接続を待ちます。接続済み旧版が自己再読み込み非対応と明示している場合だけ、ブラウザの拡張機能画面での手動更新を案内します。
