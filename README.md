...
初回は`setup-voice-env.cmd`を実行してください（内部でセットアップ画面を自動起動します）。`LocalVoiceBridge.exe`はこのセットアップの成果物として生成されるため、新規クローン直後にはまだ存在しません。2回目以降・修復時は、生成済みの`LocalVoiceBridge.exe --setup`でも同じセットアップ画面を開けます。いずれの方法でも、内部のセットアップスクリプトを直接操作する必要はありません。
