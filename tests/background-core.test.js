...
test('manifest uses the core-backed service worker entry', () => {
  ...
  const controlPollPolicy = fs.readFileSync(path.join(root, 'extension', 'background-control-poll-policy.js'), 'utf8');
  ...
  assert.match(entry, /background-control-poll-policy\.js/);
  ...
  assert.match(controlPollPolicy, /if\s*\(!micConversationEnabled\)\s*return\s*5000/);
  assert.match(controlPollPolicy, /FAST_MIC_PHASES[\s\S]*\?\s*100\s*:\s*500/);
  assert.match(statePublisher, /heartbeatMs[^\n]*30000/);
  assert.match(background, /Math\.max\(100,\s*Math\.min\(5000,\s*requestedDelay\)\)/);
});
