...
function createHarness() {
  ...
  const controller = createAutoSpeechController({
    ...
    canFinalizePreview: (preview, options) => preview.length >= options.minChars || options.completionConfirmed,
...
test('new stable reply queues exactly one Auto preview and one completion marker', () => {
  const harness = createHarness();
...
test('short streaming fragment waits for completion evidence before Auto', () => {
  const harness = createHarness();
...
test('mutation scheduling collapses bursts and inspects immediately when generation ends', () => {
  ...
  const second = { key: 'ended', text: '生成終了後にすぐ確認される返答です。', dataset: {}, complete: true };
